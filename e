...
--// Create tap visuals (dot and outline)
local function createTapVisual(position)
    ...
    if TapConfig.OUTLINE_ENABLED then
        outline = makeCircle(
            TapConfig.DOT_SIZE + (TapConfig.OUTLINE_THICKNESS * 2),
            TapConfig.DOT_COLOR,
            TapConfig.OUTLINE_TRANSPARENCY,
            TapConfig.Z_INDEX - 1
        )
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if TapConfig.IGNORE_PROCESSED and gameProcessed then return end
...
--// Handle Manual Tap Move
UserInputService.InputChanged:Connect(function(input)
    local visuals = activeDots[input]
    if visuals and visuals.dot then
        local pos = UDim2.fromOffset(input.Position.X + TapConfig.POSITION_OFFSET.X, input.Position.Y + TapConfig.POSITION_OFFSET.Y)
        visuals.dot.Position = pos
        if visuals.outline then visuals.outline.Position = pos end
    end
end)

--// Handle Manual Tap End (fade out)
...
function shoot()
	...
	task.spawn(function()
        ...
        --// TAP VISUAL INTEGRATION
        local screenPos, onScreen = Camera:WorldToScreenPoint(actualImpactPoint)
        if onScreen then
            createAutomatedTap(Vector2.new(screenPos.X, screenPos.Y))
        end
        --// END INTEGRATION
...
if script and script:IsA("Script") then
	script.Destroying:Connect(function()
		...
		if Storage and Storage.Parent then Storage:Destroy() end
		if fovCircle and fovCircle.Destroy then fovCircle:Destroy(); fovCircle = nil end
        if tapScreenGui and tapScreenGui.Parent then tapScreenGui:Destroy() end
	end)
end
